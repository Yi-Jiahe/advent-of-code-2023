...
fn identify_card_point_value(card: &str) -> usize {
    ...
    let intersection: HashSet<_> = card_numbers.intersection(&winning_numbers).collect();

    if intersection.is_empty() {
        return 0;
    }
    2_usize.pow((intersection.len() - 1).try_into().unwrap())
}
...
mod tests {
    ...
    fn test_day_4_total_scratchcard_points() {
        assert_eq!(13, day_4_total_scratchcard_points(EXAMPLE_1));
    }
}
